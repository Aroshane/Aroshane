<!-- HERO BANNER -->
<img width="100%" src="https://capsule-render.vercel.app/api?type=waving&color=gradient&customColorList=6,11,20&height=180&section=header&text=Aromal%20A%20Kurup&fontSize=42&fontColor=fff&animation=twinkling&fontAlignY=32&desc=ECE%20Student%20%7C%20Embedded%20Systems%20%26amp%3B%20IoT%20Enthusiast&descAlignY=62&descSize=16" alt="banner"/>


<!-- PROFILE VIEWS + SOCIAL BADGES -->
<p align="center">
  <a href="https://www.linkedin.com/in/aromal-a-kurup-44949931b/?originalSubdomain=in">
    <img src="https://img.shields.io/badge/LinkedIn-aromal--a--kurup-0A66C2?style=flat-square&logo=linkedin&logoColor=white"/>
  </a>
  <a href="mailto:aromalakurup2028@ec.ajce.in">
    <img src="https://img.shields.io/badge/Email-aromalakurup2028%40ec.ajce.in-EA4335?style=flat-square&logo=gmail&logoColor=white"/>
  </a>
  <img src="https://komarev.com/ghpvc/?username=Aroshane&label=Profile%20Views&color=70a5fd&style=flat-square" alt="profile views"/>
</p>

---

<!-- TYPING HEADER -->
<p align="center">
  <img src="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=700&size=28&duration=3000&pause=1000&color=70A5FD&center=true&vCenter=true&width=750&lines=Hey+there!+I'm+Aromal+%F0%9F%91%8B;ECE+Student+%7C+3rd+Year+%40+AJCE;Embedded+Systems+%2B+IoT+Builder;Learning+One+Circuit+at+a+Time+%F0%9F%94%8C" alt="Typing SVG"/>
</p>

---

<!-- CODE BLOCK ABOUT ME -->
<img align="right" width="380" src="https://raw.githubusercontent.com/abhisheknaiidu/abhisheknaiidu/master/code.gif" alt="coding gif"/>

```python
class AromalAKurup:
    def __init__(self):
        self.name       = "Aromal A Kurup"
        self.location   = "Kollam, Kerala, India 🇮🇳"
        self.degree     = "B.Tech – Electronics & Communication Engg."
        self.college    = "Amal Jyothi College of Engineering"
        self.cgpa       = 8.7
        self.year       = "3rd Year (2023 – 2027)"

        self.stack = [
            "Embedded C", "C++", "Python",
            "Arduino", "ESP32", "MATLAB",
            "Blynk IoT", "Git"
        ]

        self.currently_learning = [
            "Machine Learning 🤖",
            "Advanced IoT Systems 📡",
            "RTOS & Microcontroller Internals ⚙️",
            "Defence Drone Technology 🛸"
        ]

        self.fun_fact = "I once programmed a robot to feed fish 🐟 & advanced kitchen monitoring system"
        

    def motto(self):
        return "Build things that solve real problems. 🔧"

me = AromalAKurup()
print(me.motto())
```

<br clear="right"/>

---

<!-- TECH BADGES -->
## 🛠️ Tech Stack

**Languages**

**Tools & Platforms**

![Arduino](https://img.shields.io/badge/Arduino-00979D?style=flat-square&logo=arduino&logoColor=white)
![ESP32](https://img.shields.io/badge/ESP32-E7352C?style=flat-square&logo=espressif&logoColor=white)
![MATLAB](https://img.shields.io/badge/MATLAB-0076A8?style=flat-square&logo=mathworks&logoColor=white)
![Blynk](https://img.shields.io/badge/Blynk%20IoT-23C48E?style=flat-square&logo=blynk&logoColor=white)
![Git](https://img.shields.io/badge/Git-F05032?style=flat-square&logo=git&logoColor=white)
![GitHub](https://img.shields.io/badge/GitHub-181717?style=flat-square&logo=github&logoColor=white)
![STM32](https://img.shields.io/badge/STM32-65C2F5?style=flat-square&logo=espressif&logoColor=white)

**Hardware**

![Arduino Nano](https://img.shields.io/badge/Arduino%20Nano-00979D?style=flat-square&logo=arduino&logoColor=white)
![Microcontrollers](https://img.shields.io/badge/Microcontrollers-555555?style=flat-square&logo=raspberrypi&logoColor=white)
![Sensors](https://img.shields.io/badge/Sensors%20%26%20Actuators-38BDAE?style=flat-square&logo=esphome&logoColor=white)

**Concepts**

![IoT](https://img.shields.io/badge/IoT%20Systems-BF91F3?style=flat-square&logo=mqtt&logoColor=white)
![Embedded Systems](https://img.shields.io/badge/Embedded%20Systems-70A5FD?style=flat-square&logo=arm&logoColor=white)
![Circuit Design](https://img.shields.io/badge/Circuit%20Design-F7B731?style=flat-square&logo=circuitverse&logoColor=white)

---

<!-- GITHUB STATS -->
## 📊 GitHub Stats

<div align="center">
  <a href="https://github.com/Aroshane">
    <img height="170" src="https://github-readme-stats.vercel.app/api?username=Aroshane&show_icons=true&theme=tokyonight&hide_border=true&include_all_commits=true&count_private=true&bg_color=0d1117&title_color=70a5fd&icon_color=bf91f3&text_color=c9d1d9&border_radius=10" alt="GitHub Stats"/>
  </a>
  <a href="https://github.com/Aroshane">
    <img height="170" src="https://github-readme-stats.vercel.app/api/top-langs/?username=Aroshane&layout=compact&theme=tokyonight&hide_border=true&bg_color=0d1117&title_color=70a5fd&text_color=c9d1d9&langs_count=8&border_radius=10" alt="Top Languages"/>
  </a>
</div>

---

<!-- STREAK STATS -->
## 🔥 Streak Stats

<div align="center">
  <img src="https://streak-stats.demolab.com?user=Aroshane&theme=tokyonight-duo&hide_border=true&background=0D1117&ring=70A5FD&fire=BF91F3&currStreakLabel=70A5FD&sideLabels=38BDAE&dates=8B949E&currStreakNum=C9D1D9&sideNums=C9D1D9&stroke=0D1117&border_radius=10" alt="Streak Stats"/>
</div>

---

<!-- ACTIVITY GRAPH -->
## 📈 Activity Graph

<div align="center">
  <img src="https://github-readme-activity-graph.vercel.app/graph?username=Aroshane&theme=tokyo-night&bg_color=0d1117&color=70a5fd&line=bf91f3&point=38bdae&area=true&hide_border=true" alt="Activity Graph"/>
</div>

---

<!-- TROPHY WALL -->
## 🏆 GitHub Trophies

<div align="center">
  <img src="https://github-profile-trophy.vercel.app/?username=Aroshane&theme=tokyonight&no-frame=true&no-bg=true&row=1&column=7&margin-w=4" alt="Trophies"/>
</div>

---

<!-- WORK EXPERIENCE -->
## 💼 Experience & Activities

<details>
<summary>🛸 <strong>Workshop Participant — Defence Drone Technology & Integration</strong> | ISRO & I-STEM Initiative | 2024</summary>

> ![ISRO](https://img.shields.io/badge/ISRO-navy?style=flat-square) ![I-STEM](https://img.shields.io/badge/I--STEM-blue?style=flat-square) ![Drones](https://img.shields.io/badge/Drone%20Technology-BF91F3?style=flat-square)

- Participated in a specialized workshop associated with **ISRO** and the **I-STEM Initiative** focused on defence drone technology and integration.
- Gained exposure to real-world aerospace-grade drone systems, navigation, and payload integration concepts.
- Explored the intersection of embedded systems and national defence technology.
- Networked with engineers and researchers working at the frontier of Indian aerospace innovation.

</details>

<details>
<summary>🎓 <strong>ECE Student (Active)</strong> | Amal Jyothi College of Engineering | 2023 – Present | Kanjirapally, Kerala</summary>

> ![Embedded C](https://img.shields.io/badge/Embedded%20C-A8B9CC?style=flat-square) ![Arduino](https://img.shields.io/badge/Arduino-00979D?style=flat-square) ![ESP32](https://img.shields.io/badge/ESP32-E7352C?style=flat-square) ![Python](https://img.shields.io/badge/Python-3776AB?style=flat-square) ![MATLAB](https://img.shields.io/badge/MATLAB-0076A8?style=flat-square)

- Maintaining a strong **CGPA of 8.7** through S3, demonstrating consistent academic excellence in ECE coursework.
- Built **2 hands-on hardware projects** integrating Arduino, ESP32, IoT platforms, and real-world sensors.
- Completed certifications in **Machine Learning, MATLAB, and HP LIFE professional skills**.
- Actively seeking semester internship opportunities in Embedded Systems / IoT / Hardware Engineering.

</details>

---

<!-- FEATURED PROJECTS -->
## 🚀 Featured Projects

<div align="center">

| Project | Stack | Highlights |
|:-------:|:-----:|:----------:|
| 🐟 **Automated Fish Feeder** | Arduino Nano · Servo Motor · Embedded C · Timer Circuit | Fully automated, timer-driven fish feeding system. Eliminated manual feeding via scheduled servo actuation. |
| 🍳 **Advanced Kitchen Monitoring System** | ESP32 · Blynk IoT · Gas Sensor · Temperature Sensor | Real-time remote kitchen safety monitoring. Gas leakage & temperature alerts pushed to mobile app. |

</div>

---

<!-- ACHIEVEMENTS -->
## 🏅 Achievements & Certifications

<div align="center">

| 🏆 | Achievement | Details |
|:--:|:-----------:|:-------:|
| 🛸 | **Defence Drone Technology Workshop** | Associated with ISRO & I-STEM Initiative |
| 📐 | **MATLAB Onramp** | Official MathWorks certification |
| 🤖 | **Machine Learning Specialization** | Supervised & unsupervised learning |
| 💼 | **Resume Writing & Job Interviewing** | HP LIFE Online Course — HP Foundation |
| 🎓 | **Academic Excellence** | CGPA: **8.7 / 10** at Amal Jyothi College of Engineering |

</div>

---

<!-- EDUCATION -->
## 🎓 Education

<div align="center">

| Degree | Institution | Duration | Score |
|:------:|:-----------:|:--------:|:-----:|
| B.Tech – Electronics & Communication Engineering | Amal Jyothi College of Engineering, Kanjirapally | 2023 – 2027 (Expected) | CGPA: **8.7** |

</div>

---

<!-- CURRENTLY LEARNING -->
## 📚 Currently Learning

```
🤖 Machine Learning        → Supervised Learning, Neural Networks, Scikit-Learn
📡 Advanced IoT            → MQTT Protocol, Cloud Integration, Edge Computing
⚙️  RTOS & µC Internals    → FreeRTOS, Interrupts, Memory Management on ARM
🛸 Drone Technology        → Flight Controllers, ESC, Navigation & Payload Systems
🐍 Python (Intermediate+)  → NumPy, Pandas, Data Analysis
```

---

<!-- CONNECT -->
## 📬 Let's Connect!

<p align="center">
  <a href="https://linkedin.com/in/aromal-a-kurup">
    <img src="https://img.shields.io/badge/LinkedIn-Connect-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white"/>
  </a>
  &nbsp;
  <a href="mailto:aromalakurup2028@ec.ajce.in">
    <img src="https://img.shields.io/badge/Email-Say%20Hello-EA4335?style=for-the-badge&logo=gmail&logoColor=white"/>
  </a>
</p>

<p align="center">
  <em>"Build things that solve real problems. 🔧"</em>
</p>

---

<!-- FOOTER WAVE -->
<img width="100%" src="https://capsule-render.vercel.app/api?type=waving&color=gradient&customColorList=6,11,20&height=120&section=footer&animation=twinkling" alt="footer"/>
